...
class AuthBackend:
    ...
    def authenticate(self, identification=None, password=None):
        auth_user = None
        try:
            # Admin logging as user?
            admin, uuser = [uname.strip() for uname in identification.split(" as ")]

            # Check if admin exists and authenticates
            admin_obj = User.objects.get(username=admin)
            if (admin_obj.is_superuser or (IMPOSTOR_GROUP and IMPOSTOR_GROUP in admin_obj.groups.all())) and admin_obj.profile.check_password(password):
                try:
